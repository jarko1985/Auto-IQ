...
export function VendorApprovalQueueView({ initialVendors }: Props) {
  ...
  async function toggleExpand(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        const body = (await res.json()) as { data: VendorDetail };
        setDetail(body.data);
      }
    } finally {
  ...
  async function approve(id: string) {
    setActioningId(id);
    try {
      const res = await fetch(`/api/v1/admin/vendors/${id}/approve`, { method: "POST" });
      if (!res.ok) {
        const body = (await res.json()) as { error?: { message?: string } };
        toast.error(body.error?.message ?? "Failed to approve vendor.");
        return;
      }
  ...
  async function reject(id: string) {
    ...
    setActioningId(id);
    try {
      const res = await fetch(`/api/v1/admin/vendors/${id}/reject`, {
        method: "POST",
      ...
      if (!res.ok) {
        const body = (await res.json()) as { error?: { message?: string } };
        toast.error(body.error?.message ?? "Failed to reject vendor.");
        return;
      }
      setVendors((prev) => prev.filter((v) => v.id !== id));
      setExpandedId(null);
      ...
      >
        {vendors.map((v, i) => {
          ...
          return (
            <div
              key={v.id}
              style={{ borderBottom: i < vendors.length - 1 ? "1px solid var(--border)" : "none" }}
            >
              <div
                role="button"
                tabIndex={0}
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") toggleExpand(v.id);
                }}
                style={{
                  width: "100%",
                  display: "flex",
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      fontSize: "0.75rem",
                      fontWeight: 600,
                      cursor: actioningId === v.id ? "not-allowed" : "pointer",
                      opacity: actioningId === v.id ? 0.6 : 1,
                    }}
                  >
                    Approve
                  </button>
                ...
                <div style={{ padding: "0 1.25rem 1.25rem", borderTop: "1px solid var(--border)" }}>
                  {loadingDetail || !detail ? (
                    ...
                    <div style={{ paddingTop: "1rem" }}>
                      ...
                      >
                        <div>
                          <div style={{ color: "#8a92a6" }}>Business Type</div>
                          <div style={{ color: "#081a2f", fontWeight: 600 }}>
                            {detail.businessType.replaceAll("_", " ")}
                          </div>
                        </div>
                        <div>
